...
- [x] 2.1 In `apps/web/app/formations/[slug]/page.tsx`, update `buyNow()` function: when `data.mock === true`, use `router.push("/formations/succes?session_id=" + data.sessionId)` instead of `window.location.href = data.url`. Keep `window.location.href = data.url` for real Stripe sessions.
...
- [x] 3.1 In `apps/web/app/formations/(apprenant)/panier/page.tsx`, update `checkout()` function: when `data.mock === true`, use `router.push("/formations/succes?session_id=" + data.sessionId)` instead of `window.location.href = data.url`.
...
- [x] 4.2 Update `addToCart()`: on success, show toast "Formation ajoutée au panier !" before `router.push("/formations/panier")`. On API error (res.ok false), show error toast with the error message.
...
- [x] 5.1 Test the complete flow manually: open a formation → click "Ajouter au panier" → verify toast appears → verify formation appears in `/formations/panier` → click "Passer la commande" → verify redirect to `/formations/succes` → verify enrollment exists in `/formations/mes-formations`.
