...
Le système DOIT fournir un layout racine `app/formations/layout.tsx` qui remplace complètement le layout FreelanceHigh. Quand un utilisateur navigue sur `/*`, il NE DOIT voir aucun élément de navigation FreelanceHigh (navbar, footer, sidebar FreelanceHigh).
...
- **THEN** il est redirigé vers `/` et voit uniquement le header formations avec les menus spécifiques formations — aucun menu FreelanceHigh n'est visible

#### Scenario: Navigation directe vers une URL formations
- **WHEN** un utilisateur accède directement à `/explorer`
...
- **WHEN** un apprenant connecté accède à `/`
...
- **WHEN** un utilisateur est sur une page publique formations (`/`, `/explorer`, etc.)
...
- **WHEN** un utilisateur sur mobile (< 768px) clique sur le menu hamburger sur `/`
