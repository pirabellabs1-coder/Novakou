...
Le système DOIT afficher une sidebar latérale gauche dans l'espace apprenant (`/formations/(apprenant)/*`) avec les liens suivants : Mes Formations, Certificats, Favoris, Panier, Paramètres. La sidebar DOIT utiliser les couleurs FreelanceHigh et inclure l'avatar de l'utilisateur.

#### Scenario: Apprenant voit sa sidebar sur desktop
- **WHEN** un apprenant connecté accède à `/formations/mes-formations` sur un écran >= 1024px
...
- **THEN** il est redirigé vers `/formations/certificats` et le lien "Certificats" est visuellement actif dans la sidebar
...
Le système DOIT afficher une sidebar latérale gauche dans l'espace instructeur (`/formations/(instructeur)/*`) avec les liens suivants : Dashboard, Mes Formations, Créer une Formation, Apprenants, Revenus, Avis, Statistiques, Paramètres.

#### Scenario: Instructeur voit sa sidebar sur desktop
- **WHEN** un instructeur connecté accède à `/formations/instructeur/dashboard` sur un écran >= 1024px
...
- **THEN** il est redirigé vers `/formations/instructeur/revenus` et le lien "Revenus" est visuellement actif dans la sidebar
...
- **WHEN** un visiteur non connecté navigue sur `/formations`
...
- **WHEN** un apprenant connecté navigue sur `/formations/explorer` (page publique)
...
- **WHEN** un instructeur connecté navigue sur `/formations` (page publique)
...
- **WHEN** un apprenant est sur `/formations/certificats`
...
- **WHEN** un visiteur est sur `/formations/explorer`
...
- **WHEN** un apprenant connecté clique sur "Mes Formations" depuis `/formations/explorer`
...
- **WHEN** un apprenant est sur `/formations/apprendre/123`
...
- **WHEN** un instructeur est sur `/formations/instructeur/revenus`
