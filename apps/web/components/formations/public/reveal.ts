/**
 * Révélation en cascade des pages publiques — anime.js à la demande.
 *
 * Même mécanique que components/home/reveal-compteurs.ts (accueil) et
 * components/formations/explorer/reveal.ts (marketplace), pour la classe
 * `.nkp-reveal` (état initial dans public.css) : 550 ms, 60 ms de cascade,
 * 20 px de montée. anime.js n'est chargé (sous-chemins, pas le paquet
 * entier) qu'au premier élément visible. Le HTML porte déjà le contenu ;
 * en prefers-reduced-motion ou sans IntersectionObserver, on pose l'état
 * final sans bouger. Transform et opacity uniquement ; `will-change` posé
 * juste avant, retiré à la fin.
 */
...
export const CLASSE_REVEAL = "nkp-reveal";
...
/** Même courbe que --nkp-ease dans public.css. */
...
/**
 * Observe les `.nkp-reveal` pas encore révélés sous `root`. Ceux qui entrent
