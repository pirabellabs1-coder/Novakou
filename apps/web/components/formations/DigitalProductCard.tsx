    ...
    >
      <div className="relative overflow-hidden aspect-[4/3] bg-slate-100 dark:bg-slate-800 dark:bg-slate-700">
        ...
        <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
          <span className="bg-indigo-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{t(`product_type_${product.productType}`)}</span>
          {product.previewEnabled && (
            <span className="bg-sky-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{t("product_preview_available")}</span>
          )}
        </div>
        {discount && (
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-bold text-sm line-clamp-2 mb-1 group-hover:text-primary transition-colors">{title}</h3>
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">{product.instructeur.user.name}</p>
        {product.rating > 0 && (
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mt-auto pt-2 border-t border-slate-100 dark:border-slate-700">
          <span>{product.salesCount} {salesLabel}</span>
          {isLimited && (
            <>
              <span>&middot;</span>
              <span className={stockPct < 20 ? "text-red-500 font-bold" : "text-orange-500"}>{t("product_limited")}</span>
            </>
          )}
        </div>
          ...
          <div>
            {product.isFree ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-lg font-extrabold text-slate-900 dark:text-white">{product.price.toFixed(2)}&euro;</span>
                {discount && <span className="text-sm text-slate-400 line-through">{product.originalPrice!.toFixed(2)}&euro;</span>}
              </div>
            )}
          </div>
