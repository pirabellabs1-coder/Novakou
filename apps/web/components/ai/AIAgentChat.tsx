...
export default function AIAgentChat({
  ...
  // L'IA répond via notre serveur : aucun SDK externe à charger, donc aucune
  // attente et aucun blocage possible par un bloqueur de publicité. On garde
  // seulement de quoi signaler une indisponibilité réelle (quota, panne).
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const reponse = (await chatIA(fullPrompt, { usage: "agent" })).trim();
      setIndisponible(null);
      setMessages(prev => [...prev, { role: "assistant", content: reponse || "Désolé, je n'ai pas compris. Pouvez-vous reformuler ?" }]);
    } catch (e) {
      // Le serveur rédige déjà un message compréhensible (quota du jour
      // atteint, assistant coupé) : on l'affiche tel quel.
      const raison = e instanceof Error && e.message ? e.message : "L'assistant n'a pas pu répondre.";
      setIndisponible(raison);
      setMessages(prev => [...prev, { role: "assistant", content: raison }]);
    } finally {
      setSending(false);
    }
  }
      ...
      <div className="mb-5">
        ...
        {indisponible && (
          <div className="flex items-center gap-1.5 text-[11px]">
            ...
            <span className="text-[#5c647a]">{indisponible}</span>
          </div>
        )}
      </div>
