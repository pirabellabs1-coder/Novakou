...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.user.count({ where: { role: { not: "ADMIN" }, status: "ACTIF", registrationSource: { not: "formations" } } }),
      prisma.user.count({ where: { role: "FREELANCE", status: "ACTIF", registrationSource: { not: "formations" } } }),
      prisma.user.count({ where: { role: "CLIENT", status: "ACTIF", registrationSource: { not: "formations" } } }),
      prisma.user.count({ where: { role: "AGENCE", status: "ACTIF", registrationSource: { not: "formations" } } }),
      prisma.order.count(),
