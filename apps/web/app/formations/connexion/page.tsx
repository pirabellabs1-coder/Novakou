...
export default function FormationsConnexionPage() {
  ...
  async function redirectAfterLogin() {
    // Update the formations role to match what was selected on the login page
    try {
      ...
      console.error("[LOGIN] Error updating formations role:", err);
    }
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md space-y-8">
        ...
        {/* Role selection */}
        <div>
          <label className="block text-sm font-semibold mb-3 text-slate-900 dark:text-white">Se connecter en tant que</label>
          <div className="grid grid-cols-2 gap-3">
            {(["apprenant", "instructeur"] as const).map((r) => (
              ...
              >
                ...
                {r === "apprenant" ? "Apprenant" : "Instructeur"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            onClick={() => {
              ...
              signIn("linkedin", { callbackUrl: `/formations/callback?role=${role}` });
            }}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-primary/50 transition-all text-sm font-semibold bg-white dark:bg-slate-900 dark:bg-slate-800"
          >
