...
export default function CertificateVerificationPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const [result, setResult] = useState<{ valid: boolean; revoked?: boolean; certificate?: CertificateData } | null>(null);
  ...
  const formationTitle = result?.certificate
    ? (locale === "fr" ? result.certificate.formation.titleFr : (result.certificate.formation.titleEn || result.certificate.formation.titleFr))
    : "";
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500">{locale === "fr" ? "Vérification en cours..." : "Verifying..."}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-12 px-4">
      <div className="max-w-2xl mx-auto">
        {/* Back link */}
        <Link href="/formations" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-primary mb-8 transition-colors">
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-block">
            <span className="text-2xl font-black tracking-tight text-primary">
              Freelance<span className="text-slate-900">High</span>
            </span>
          </Link>
          ...
          {result?.valid && result.certificate && (
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-xs text-slate-500 mb-0.5">{locale === "fr" ? "Délivré à" : "Issued to"}</p>
                  <p className="font-bold text-slate-900 text-lg">{result.certificate.user.name}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-bold text-slate-900">{formationTitle}</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {locale === "fr" ? "Durée :" : "Duration:"} {formatDuration(result.certificate.formation.duration)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-xs text-slate-500 mb-0.5">{locale === "fr" ? "Instructeur" : "Instructor"}</p>
                  <p className="font-bold text-slate-900">{result.certificate.instructor}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-green-50 rounded-xl text-center">
                  ...
                  <p className="text-xs text-slate-500">{locale === "fr" ? "Score obtenu" : "Score"}</p>
                  <p className="text-2xl font-bold text-green-700">{result.certificate.score}%</p>
                </div>
                <div className="p-4 bg-blue-50 rounded-xl text-center">
                  <Calendar className="w-6 h-6 text-blue-600 mx-auto mb-1" />
                  <p className="text-xs text-slate-500">{locale === "fr" ? "Date d'obtention" : "Issue date"}</p>
              ...
              {/* Link to formation */}
              <Link
                href={`/formations/${result.certificate.formation.slug}`}
