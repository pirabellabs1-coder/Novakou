...
export default function MentorPublicProfilePage({ params }: { params: { id: string } }) {
  ...
  function handleSlotClick(slot: PreviewSlot) {
    if (!session) {
      router.push(
        `/formations/connexion?callbackUrl=/formations/mentors/${params.id}/reserver?slot=${encodeURIComponent(slot.start)}`,
      );
      return;
    }
    router.push(`/formations/mentors/${params.id}/reserver?slot=${encodeURIComponent(slot.start)}`);
  }

  function handleBookClick() {
    if (!session) {
      router.push(`/formations/connexion?callbackUrl=/formations/mentors/${params.id}/reserver`);
      return;
    }
    router.push(`/formations/mentors/${params.id}/reserver`);
  }
    ...
    <div className="min-h-screen bg-[#f7f9fb]">
      ...
      <div className="bg-white border-b border-gray-100 px-4 md:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs text-[#5c647a]">
          ...
          <Link href="/formations/mentors" className="hover:text-[#006e2f] transition-colors">Mentors</Link>
          <span className="material-symbols-outlined text-[12px]">chevron_right</span>
