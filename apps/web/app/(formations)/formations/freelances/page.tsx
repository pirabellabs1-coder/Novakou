    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      <section className="pt-32 pb-16 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            <Link href="/formations/mentors" className="px-7 py-4 rounded-2xl text-white font-bold text-sm hover:opacity-90 transition-opacity shadow-lg" style={{ background: "linear-gradient(to right, #006e2f, #22c55e)" }}>
              Découvrir tous les mentors
            </Link>
            <Link href="/formations/inscription?role=instructeur" className="px-7 py-4 rounded-2xl bg-slate-100 text-[#191c1e] font-bold text-sm hover:bg-slate-200 transition-colors">
              Devenir créateur
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            {SPECIALTIES.map((s) => (
              <Link
                key={s.label}
                href={`/formations/explorer?category=${encodeURIComponent(s.label)}`}
      ...
      <section className="py-16 px-6">
        <div className="max-w-4xl mx-auto text-center bg-gradient-to-br from-[#006e2f] to-[#22c55e] rounded-3xl p-12 text-white">
          ...
          <Link href="/formations/mentors" className="inline-flex items-center gap-2 bg-white text-[#006e2f] px-7 py-4 rounded-2xl font-bold text-sm hover:bg-slate-100 transition-colors">
