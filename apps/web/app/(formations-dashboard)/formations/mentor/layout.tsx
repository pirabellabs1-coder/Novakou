...
const NAV = [
  { icon: "dashboard", label: "Tableau de bord", href: "/formations/mentor/dashboard" },
  { icon: "event", label: "Mon calendrier", href: "/formations/mentor/calendrier" },
  { icon: "event_available", label: "Mes rendez-vous", href: "/formations/mentor/rendez-vous" },
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb]" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 h-16 flex items-center px-4 md:px-6 gap-4">
        ...
        <Link href="/formations/mentor/dashboard" className="flex items-center gap-2 flex-shrink-0">
        ...
        <Link href="/formations/mentor/profil">
          {image ? (
      ...
      <aside className={`fixed top-0 left-0 bottom-0 z-40 w-64 bg-white border-r border-gray-100 pt-16 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="px-3 py-4 border-t border-gray-100">
          <button onClick={() => signOut({ callbackUrl: "/formations" })}
