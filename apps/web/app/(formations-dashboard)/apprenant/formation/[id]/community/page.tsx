...
import { ArrowLeft, Plus, MessagesSquare, MessageCircle, Pin, Check } from "lucide-react";
...
export default function CommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-5 md:p-8 max-w-4xl mx-auto">
      <Link href={`/apprenant/formation/${formationId}`} className="text-xs font-semibold text-[#5d7166] hover:text-[#13241b] inline-flex items-center gap-1 mb-4">
        <ArrowLeft size={16} />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-[#13241b]">Communauté</h1>
          <p className="text-sm text-[#5d7166] mt-1">Échangez avec l'instructeur et les autres apprenants.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-[#e4eae6] p-16 text-center">
          <MessagesSquare size={48} className="text-gray-300 mx-auto" />
          <h3 className="text-lg font-bold text-[#13241b] mt-3">Aucune discussion encore</h3>
          <p className="text-sm text-[#5d7166] mt-2">Soyez le premier à poser une question ou partager un retour.</p>
        </div>
      ) : (
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto"
            ...
            <h2 className="text-xl font-extrabold text-[#13241b] mb-4">Nouvelle discussion</h2>
            <div className="space-y-3">
              <input
                type="text"
                value={title}
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowCreate(false)} disabled={createMut.isPending}
                  className="flex-1 py-2.5 rounded-xl bg-gray-100 text-[#13241b] text-sm font-bold">
                  Annuler
                </button>
                <button
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {discussion.isResolved && (
                <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 uppercase inline-flex items-center gap-1">
                  <Check size={10} strokeWidth={3} /> Résolu
                </span>
              )}
            </div>
            <h3 className="text-base font-extrabold text-[#13241b] mb-1">{discussion.title}</h3>
            {!isOpen && (
              <p className="text-sm text-[#5d7166] line-clamp-2">{discussion.content}</p>
            )}
          </div>
          <div className="flex flex-col items-center text-[11px] text-[#5d7166] flex-shrink-0">
      ...
      {isOpen && (
        <div className="px-5 pb-5 border-t border-gray-100">
          <div className="my-4 pl-13">
            <p className="text-sm text-[#13241b] whitespace-pre-wrap">{discussion.content}</p>
          </div>
